...
Msg* TestHttpSupplier::ProcessMsg(MsgAudioDsd* aMsg)
{
    ASSERTS();
    return aMsg;
}
    ...
    : Suite(aSuiteName)
{
    ...
    std::vector<NetworkAdapter*>* ifs = Os::NetworkListAdapters(*gEnv, Environment::ELoopbackUse, true, "Loopback");
...
SuiteHttpChunked::SuiteHttpChunked()
    : Suite("Chunked HTTP")
{
    std::vector<NetworkAdapter*>* ifs = Os::NetworkListAdapters(*gEnv, Environment::ELoopbackUse, true, "SuiteHttpChunked");
    TIpAddress addr = (*ifs)[0]->Address();
    for (TUint i=0; i<ifs->size(); i++) {
