...
RaopDevice::RaopDevice(TUint aDiscoveryPort, IFriendlyNameObservable& aFriendlyNameObservable, TIpAddress aIpAddr, const Brx& aMacAddr, Net::IMdnsProvider& aProvider)
    : iProvider(aProvider)
...
RaopDiscoveryServer::RaopDiscoveryServer(Environment& aEnv, NetworkAdapter& aNif, IFriendlyNameObservable& aFriendlyNameObservable, const Brx& aMacAddr, IAttenuator& aAttenuator, Net::IMdnsProvider& aProvider)
    : iEnv(aEnv)
    , iAdapter(aNif)
    , iObserversLock("RDOL")
{
    ...
    iRaopDevice = new RaopDevice(iRaopDiscoveryServer->Port(), aFriendlyNameObservable, iAdapter.Address(), aMacAddr, aProvider);
...
RaopDiscovery::RaopDiscovery(Environment& aEnv, IPowerManager& aPowerManager, IFriendlyNameObservable& aFriendlyNameObservable, const Brx& aMacAddr, IAttenuator& aAttenuator, Net::IMdnsProvider& aProvider)
    : iEnv(aEnv)
    , iFriendlyNameObservable(aFriendlyNameObservable)
    , iMacAddr(aMacAddr)
    , iCurrent(nullptr)
...
{
    if (TIpAddressUtils::Equals(aNif1.Address(), aNif2.Address()) && TIpAddressUtils::Equals(aNif1.Subnet(), aNif2.Subnet()) && strcmp(aNif1.Name(), aNif2.Name()) == 0) {
        return true;
    }
    return false;
}
